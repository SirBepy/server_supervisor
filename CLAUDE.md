...
A local process supervisor. One headless owner (the Tauri Rust backend) launches and owns dev servers across projects (Flutter, Node, etc.), survives window close (runs in the tray), and on full quit kills every child it started. A webview dashboard is the human UI; a localhost HTTP+WS API (bearer-token) lets an AI agent list / start / stop / restart processes and read logs.
...
- The Rust backend is the SINGLE owner of every spawned process. Closing the window hides to tray (servers keep running). Tray Quit kills all children, then exits. No orphans, ever.
