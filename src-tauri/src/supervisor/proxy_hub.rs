...
impl ProxyHub {
    ...
    /// already-running listener handles - no rebind. `pub(super)`: called
    /// from the sibling `hub_lifecycle` module.
    pub(super) fn set_active(&self, preset: &UpstreamPreset) {
        *self.shared.active.write().unwrap() = ActiveUpstream::from_preset(preset);
    }

    /// Snapshot of the ring buffer, oldest first. `pub(super)`: called from
    /// the sibling `hub_lifecycle` module.
    pub(super) fn log_snapshot(&self) -> Vec<RequestLogEntry> {
